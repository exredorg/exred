...
docker save -o out/images_rpi.tar prod_exred_db_rpi prod_exred_rpi

echo -e "\n>> bundling all files together"
if [ -d "~/exred_data" ]; then
  tar -cvzf out/BUNDLE.tar.gz -C ./out images_rpi.tar -C .. env_file docker-compose.yml -C ./assets load.sh -C ~/ exred_data
else
  tar -cvzf out/BUNDLE.tar.gz -C ./out images_rpi.tar -C .. env_file docker-compose.yml -C ./assets load.sh 
fi
